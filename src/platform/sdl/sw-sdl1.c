...
#else
	SDL_SetVideoMode(renderer->viewportWidth, renderer->viewportHeight, 32,  SDL_HWSURFACE | (SDL_FULLSCREEN * renderer->player.fullscreen));
#endif

//markus
	renderer->player.newWidth = renderer->viewportWidth;
	renderer->player.newHeight = renderer->viewportHeight;
//printf ( "sw sdl1 w= %d h= %d \n",renderer->viewportWidth,renderer->viewportHeight); 
//
