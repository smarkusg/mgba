...
void mCoreConfigDirectory(char* out, size_t outLength) {
	...
	char* xdgConfigHome = "PROGDIR:mGBAConfig";
	snprintf(out, outLength, "%s", xdgConfigHome);
	mkdir(out, 0755);
	snprintf(out, outLength, "%s/%s", xdgConfigHome, "screenshots");
	mkdir(out, 0755);
	snprintf(out, outLength, "%s/%s", xdgConfigHome, "save");
	mkdir(out, 0755);
	snprintf(out, outLength, "%s/%s", xdgConfigHome, "ram");
	mkdir(out, 0755);
	snprintf(out, outLength, "%s/%s", xdgConfigHome, "patch");
	mkdir(out, 0755);
	snprintf(out, outLength, "%s/%s", xdgConfigHome, "cheats");
	mkdir(out, 0755);
//markus restoring default values
	snprintf(out, outLength, "%s", xdgConfigHome);
...
void mCoreConfigPortablePath(char* out, size_t outLength) {
	...
	out[0] = '\0';
#elif defined __AMIGAOS4__
	snprintf(out, outLength, "%s/", "PROGDIR:mGBAConfig");
#else
